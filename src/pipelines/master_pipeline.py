...
def main(argv):
    ...
    train_scores = observations.main([clusters, order, data, directory, features, vocab, encoder_decoder])
    test_scores = observations.main([test_clusters, test_order, test_data, directory, features, vocab, encoder_decoder])
...
if __name__ == '__main__':
    ...
    featureTuple = namedtuple('features','cosine, tf_idf, bog, skipthoughts')
    features = featureTuple(args.cosine, args.tf_idf, args.bag_of_words, args.skipthoughts)

    algTuple = namedtuple('algorithms','log_reg, svm')
    algorithms = algTuple(args.log_reg, args.svm)

    if not (args.cosine or args.tf_idf or args.bag_of_words or args.skipthoughts):
