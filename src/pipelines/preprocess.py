...
def main(argv):
    ...
    if 'bow' in features:
        vocab = gen_vocab(corpus_dict, **features['bow'])
    elif 'lda' in features:
        vocab = gen_vocab(corpus_dict, **features['lda'])
    elif 'wordonehot' in features:
        vocab = gen_vocab(corpus_dict, **features['wordonehot'])

    if 'lda' in features:
        lda = build_lda(trainingdata, vocab, **features['lda'])
