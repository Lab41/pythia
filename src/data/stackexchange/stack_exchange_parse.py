            ...
            for file_name in os.listdir(fullfoldername):
                if file_name.endswith(".json"):
                    ...
                    with open(full_file_name, 'r') as dataFile:
                        for line in dataFile:
                            entries += 1
                    if entries >= minpost and entries <= maxpost:
                        filestokeep.append((full_file_name, foldername))
...
def main(args):
    ...
    if args.filter or args.skipparse:
        filter_json_files(directory, corpus_directory, int(args.minpost), int(args.maxpost))
